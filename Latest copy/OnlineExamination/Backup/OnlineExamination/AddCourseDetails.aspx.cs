...
namespace OnlineExamination
{
    public partial class AddCourseDetails : System.Web.UI.Page
    {
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            string hodName = txtHODName.Text.ToString();
            decimal fees = decimal.Parse(txtCourseFees.Text.ToString());
            ...
            {
                sQuery = "insert into CourseDetails (CourseFees,CourseName,HODName) values (" + fees + ",'" + courseName + "','" + hodName + "')";
            }
            else if (btnSubmit.Text == "Update")
            {
                int courseId = int.Parse(Application["CourseID"].ToString());
                sQuery = "update CourseDetails set CourseFees=" + fees + ", HODName='" + hodName + "', CourseName='" + courseName + "' where CourseId=" + courseId;
        ...
        private void BindCourseDetails()
        {
            ...
            string sQuery = "select CourseId,CourseName,HODName,CourseFees from CourseDetails";
            SqlCommand cmd = new SqlCommand(sQuery, _sConn);
        ...
        protected void gvCourseDetails_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (e.CommandName == "Edit")
            {
                ...
                txtCourseFees.Text = gvCourseDetails.Rows[index].Cells[2].Text.ToString();
                txtCourseName.Text = gvCourseDetails.Rows[index].Cells[0].Text.ToString();
                txtHODName.Text = gvCourseDetails.Rows[index].Cells[1].Text.ToString();
