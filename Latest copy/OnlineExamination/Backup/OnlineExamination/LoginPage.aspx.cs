...
namespace OnlineExamination
{
    public partial class LoginPage : System.Web.UI.Page
    {
        ...
        protected void btnLogin_Click(object sender, EventArgs e)
        {
            ...
            {
                sQuery = "select StudentId,StudentName,MobileNumber,EmailId from StudentDetails where UserName='" + username + "' and PassWord='" + passWord + "'";
            }
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    {
                        bool isRight = CheckForMacAddress();
                        if (isRight == true)
                        {
        ...
        private bool CheckForMacAddress()
        {
            bool isRight = false;
            NetworkInterface[] findMac = NetworkInterface.GetAllNetworkInterfaces();
            string macAddress = findMac[0].GetPhysicalAddress().ToString();
            if (_sconn.State != ConnectionState.Open)
            {
                _sconn.Open();
            }
            string sQuery = "select MacAddress from MacAddress where MacAddress='" + macAddress + "'";
            SqlCommand cmd = new SqlCommand(sQuery, _sconn);
            SqlDataAdapter adp = new SqlDataAdapter(cmd);
            DataTable dt = new DataTable();
            adp.Fill(dt);
